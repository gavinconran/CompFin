...
plot(sd.vec, mu.vec, pch=16, col="blue",
     ylim=c(0, max(mu.vec) + .04), xlim=c(0, max(sd.vec) + 0.07))
text(x=sd.vec[1], y=mu.vec[1], labels="MSFT", cex=0.8, pos=4)
text(x=sd.vec[2], y=mu.vec[2], labels="NORD", cex=0.8, pos=4)
text(x=sd.vec[3], y=mu.vec[3], labels="SBUX", cex=0.8, pos=4)
#
...
var.gmin.ns
sig.gmin.ns = sqrt(var.gmin.ns)
points(sig.gmin.ns, er.gmin.ns, type="b",  col="black", pch=16)
text(x=sig.gmin.ns, y=er.gmin.ns, labels="Global Min Var",cex=0.8, pos=2)
