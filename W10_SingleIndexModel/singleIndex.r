...
# extractor functions
coef(msft.fit)
residuals(msft.fit)[1:5]  # epsilon
fitted(msft.fit)[1:5] # Fitted regression line, i.e. Return, R for asset i
...
gmin.port.sample = globalMin.portfolio(mu.hat, cov.hat)
gmin.port.sample

# use single index covariance
gmin.port.si = globalMin.portfolio(mu.hat, cov.si)
gmin.port.si
...
# Test for constant parameters
# summary method - gives SE values, t-stats etc
# the t values are t-statistics for testing the hypothesis
# that the true coefficient is equal to zero
summary(msft.fit)
